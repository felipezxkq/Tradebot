...
class JsonSocket(object):
    ...
    def _sendObjDebug(self, obj):
        msg = json.dumps(obj)
        self._waitingSendDebug(msg)
    ...
    def _waitingSendDebug(self, msg):
        if self.socket:
            sent = 0
            msg = msg.encode('utf-8')
    ...
    def _read_and_respond(self, bytesSize=4096):
        if not self.socket:
            raise RuntimeError("socket connection broken")
        while True:
            print("Recibiendo respuesta")
            char = self.conn.recv(bytesSize)
            #self._receivedData = char
            print("Se recibió: ")
            print(self._receivedData)
            try:
                print("Tratando de sacar resp y size")
                print("El tipo de receivedData es: ", type(char))
                resp = json.loads(char)
                #print(resp)
                return resp
                size = len(resp)
                #(resp, size) = self._decoder.raw_decode(self._receivedData)
                ...
                if size == len(self._receivedData):
                    print("El tamanio es igual")
                    self._receivedData = ''
                    break
                elif size < len(self._receivedData):
                    print("El tamanio es menor")
                    self._receivedData = self._receivedData[size:].strip()
                    break
            except ValueError as e:
                continue
    ...
    def _readObjAndRespond(self):
        msg = self._read_and_respond()
        return msg
...
class APIClient(JsonSocket):
    ...
    def executeAndRespond(self, dictionary):
        self._sendObjDebug(dictionary)
        return self._readObjAndRespond()
...
def main():
    ...
    #sclient.subscribePrices(['ZECBTC'])
    
    #sclient.subscribeProfits()

    #sclient.execute(dict(command='getKeepAlive', streamSessionId=sclient._ssId))
    ...
    #client.commandExecute('getSymbol', dict(symbol="ZECBTC"))

    startTime = client.commandExecute('getServerTime')
    startTime = startTime['returnData']['time'] - 600000

    client.commandExecute('getChartLastRequest',  dict(info = dict(period= 1, start = startTime, symbol='ZECBTC')))
